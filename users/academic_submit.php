...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    echo $rs[2];
    if($rs){
        $stmt4 = $pdo->prepare("UPDATE user_info1 SET  full_name=:fname, dob=:dob, gender=:gen,profile_pic_name=:pfp_name, profile_pic_path=:pfp_path, 
        bio=:bio,graduation_year=:grad_yr,course_degree=:course_degree,specialization=:specialization,u_email=:email WHERE user_id=$user_id");
...
$stmt1 = $pdo->prepare("INSERT INTO user_info1 (full_name,dob, gender,profile_pic_name,profile_pic_path,bio, graduation_year, course_degree, specialization,u_email,
user_id) VALUES (:full_name, :dob, :gender,:prof_pic_name,:prof_pic_path,:bio,:grad_yr,:course,:specialization,:mail,:uid)");
...
$stmt1->bindParam(":uid",$user_id,PDO::PARAM_INT);
if ($stmt1->execute()) {
    // session_destroy();
    header('location:academic.php');
} else {
